...
function PongGame() {
  // 1. useRef pour cibler notre canvas dans le code
  const canvasRef = useRef(null);
  const [score, setScore] = useState({ player: 0, player2: 0});

  // 2. useEffect pour contenir toute la logique de pong_web.js
  useEffect(() => {
      //Game constant
    const HEIGHT = 60;
    const WIDTH = 80;
    ...
    function moveBall(){
    	move_obj_ball(ball);
    	if (ball.x < 0){
    	  setScore(currentScore => ({ ...currentScore, player2: currentScore.player2 + 1 }));
    	  ...
    	  setScore(currentScore => ({ ...currentScore, player1: currentScore.player1 + 1 }));
    		ball.x = Math.floor(WIDTH / 2);
    		ball.y = Math.floor(HEIGHT / 2);
    		let angle = (Math.random() - 0.5) * MAX_BOUNCE_ANGLE;
    		let dir = Math.random() < 0.5 ? 1 : -1;
    ...
    return () => {
      console.log("Nettoyage du jeu Pong...");
      window.cancelAnimationFrame(animationFrameId); // Arrête la boucle de jeu
      window.removeEventListener('keydown', handleKeyDown); // Supprime l'écouteur d'événement
      // Si tu as une connexion WebSocket, ferme-la ici:
      // socket.close();
    };
    
    }, []); // Le [] vide signifie que ce code ne s'exécute qu'une seule fois

  // 3. Le JSX retourne le canvas que notre jeu utilisera
  return (
    <div className="pong-container">
      <div id="score">Player 1: 0 &mdash; Player 2: 0</div>
